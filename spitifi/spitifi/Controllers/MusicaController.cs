...
namespace spitifi.Controllers
{
    public class MusicaController : Controller
    {
        ...
        public async Task<IActionResult> Create([Bind("Nome,AlbumFK,DonoFK")] Musica musica,
            [Bind("Titulo")] Album album, List<IFormFile> musicaNova, IFormFile fotoAlbum)
        {
        ...
        [Authorize(Roles = "Artista, Administrador")]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit([FromRoute] int id, [Bind("Id,Nome,AlbumFK,DonoFK")] Musica musica,
            IFormFile musicaNova)
        {
            try
            {
                string nomeAudio = "";
        ...
        public async Task<IActionResult> DeleteConfirmation(int id)
        {
            ...
            // validar se quem tenta alterar a playlist é o dono or admin
            if (musicaValidacaoAuthorization.Dono.IdentityUser != User.FindFirstValue(ClaimTypes.NameIdentifier) &&
                !User.IsInRole("Administrador"))
            {
                return Forbid();
            }
